// wizard.js - Fixed full-size images and preview navigation
(function($) {
    'use strict';
    
    // Global variables at the top level
    ...
    function showProgress(message, percent) {
        ...
        if ($progress.length) {
            ...
            $progress.find('.progress-percent').text(Math.round(percent) + '%');
            $progress.show();
        } else {
   ...
   // Handle purchase option selection for BOTH love and kids
$(document).on('click', '.purchase-option', function() {
    ...
    // Update add to cart button text (for the current step)
    if (isKids) {
        $('#step4-content-kids .add-to-cart-btn').html(`Add to Cart - ${price} DH`);
        // Visual feedback for kids options
        $('.kids-purchase').css({
            'border-color': '#ddd',
            'transform': 'scale(1)'
        });
        ...
        $('#step4-content-love .add-to-cart-btn').html(`Add to Cart - ${price} DH`);
        // Visual feedback for love options
    ...
    console.log(`Selected: ${purchaseOption} (Product ID: ${selectedProductId})`);
});
// NEW FUNCTION: Generate kids preview
...
const selections = validation.selections; // Add this line
...
Each page is created using powerful AI systems that consume real computing resources, so we only generate the full book for confirmed customers. Thank you for understanding!</p>
    ...
    generateKidsPreviewPage(); // NEW: Only generate preview
});
    ...
    generateSinglePreviewPage(); // NEW: Only generate 1 preview
});

// NEW FUNCTION: Generate only 1 preview page
...
// Get selections from validation
...
Each page is created using powerful AI systems that consume real computing resources, so we only generate the full book for confirmed customers. Thank you for understanding!</p>
        ...
        alert('Preview Generation Failed: ' + error.message);
                ...
                // Show/hide "other" fields based on initial state
        ...
        async function generateKidsStoryBook() {
            console.log('=== GENERATING KIDS STORY BOOK ===');
            ...
            // Character details
            const kidName = characterDetails.kidName;
            const kidAge = '5'; // Default age or from input
            const selections = validation.selections;
            
            console.log('Generating kids story book (3 pages)...');
            
            // Show loading
            $('#kids-story-form').hide();
            $('#kids-story-loading').show();
            $('#kids-story-results').hide();
            $('#generate-kids-story').prop('disabled', true);
            
            // Update loading text for 3 pages
            $('#kids-story-progress-text').text('Generating pages: 0/3');
            
            try {
                const totalPages = 3; // CHANGED FROM 10 TO 3
                const generatedPages = [];
                const maxRetries = 2; // Retry failed pages
                
                // Generate prompts for 3 pages
                const prompts = [];
                for (let i = 1; i <= totalPages; i++) {
                    const prompt = `Create page ${i} of ${totalPages} for a children's story about ${kidName}, age ${kidAge}. 
        Theme: ${selections.template}.

        IMPORTANT REQUIREMENTS:
        1. Image size: A5 dimensions (1748 x 2480 pixels) - portrait orientation
        2. Style: Bright cartoon illustration, clean lines, Pixar-style
        3. Leave EMPTY SPACE at bottom 20% of image for text bubble
        4. Text bubble: Will be added later - DO NOT include text in image
        5. Composition: ${kidName} should be clearly visible as main character
        6. Scene: Create a fun scene related to ${selections.template}
        7. Background: Detailed but not too busy
        8. Colors: Vibrant, child-friendly palette`;
                    
                    prompts.push(prompt);
                }
                
                console.log(`Generating ${prompts.length} pages sequentially...`);
                
                // Function to update progress bar
                function updateKidsProgress(current, total) {
                    const percent = Math.round((current / total) * 100);
                    $('#kids-story-progress-bar').css('width', percent + '%');
                    $('#kids-story-progress-text').text(`Generating pages: ${current}/${total}`);
                }
                
                // Update initial progress
                updateKidsProgress(0, totalPages);
                
                // Generate pages sequentially with retry
                for (let i = 0; i < totalPages; i++) {
                    ...
                    while (retryCount < maxRetries && !success) {
                        try {
                            console.log(`Generating page ${i + 1}/${totalPages} (Attempt ${retryCount + 1})`);
                            
                            const pageData = await generateSingleKidsPage({
                                image: imageData,
                                prompt: prompts[i],
                                pageNumber: i + 1,
                            ...
                            if (pageData) {
                                ...
                                // Update progress
                                updateKidsProgress(i + 1, totalPages);
                            ...
                            retryCount++;
                            console.log(`✗ Page ${i + 1} failed (Attempt ${retryCount}/${maxRetries}):`, error.message);
                            ...
                            if (retryCount < maxRetries) {
                                ...
                                prompts[i] = prompts[i] + ' Make sure to include the child character clearly.';
                            } else {
                                console.log(`✗ Page ${i + 1} failed after ${maxRetries} attempts`);
                            }
                        }
                    }
                ...
                // Store the generated pages WITH text
                window.generatedKidsPages = generatedPages.map((page, index) => ({
                    page_number: page.page_number,
                    image_url: page.image_url,
                    text: `Page ${page.page_number}: ${characterDetails.kidName}'s ${selections.template} adventure continues!`,
                    prompt: page.prompt
                }));
                window.kidsSelections = selections;
                ...
                alert(`✅ Kids story generated successfully! ${generatedPages.length} pages created.`);
                ...
                console.error('Kids Story Generation Error:', error);
                $('#kids-story-loading').hide();
                $('#kids-story-form').show();
                $('#generate-kids-story').prop('disabled', false);
                
                // Even if some pages failed, show what we have
                if (window.generatedKidsPages && window.generatedKidsPages.length > 0) {
                    alert(`Partially completed: ${window.generatedKidsPages.length} pages generated. Some pages failed: ${error.message}`);
                    $('#kids-story-loading').hide();
                    $('#kids-story-results').show();
                    generateKidsBookPreview();
                    $('.add-to-cart-btn').prop('disabled', false).css('opacity', 1);
                } else {
                    alert('Kids Story Generation Failed: ' + error.message);
                }
            }
        }

        // NEW FUNCTION: Automatically show results after generation
        function showKidsStoryResults() {
            console.log('Automatically showing kids story results...');
            
            // Hide the form and loading
            $('#kids-story-form').hide();
            $('#kids-story-loading').hide();
            $('#generate-kids-story').hide();
        ...
        // Generate the complete love story book - UPDATED FOR 3 PAGES
        async function generateLoveStoryBook() {
            console.log('Generating love story book (3 pages only - TEST MODE)...');
            ...
            // Update loading text for 3 pages
            $('#story-progress-text').text('Generating pages: 0/3 (TEST MODE)');
            ...
            try {
                ...
                // Generate prompts for 3 pages only
                const pagePrompts = generateLovePagePrompts(validation.selections);
                
                console.log(`Generating ${pagePrompts.length} pages sequentially with retry...`);
                
                // Initialize progress
                let generatedPages = [];
                let completedPages = 0;
                let totalPages = pagePrompts.length;
                const maxRetries = 2;
                
                // Function to update progress bar
                function updateProgress(current, total) {
                    const percent = Math.round((current / total) * 100);
                    $('#story-progress-bar').css('width', percent + '%');
                    $('#story-progress-text').text(`Generating pages: ${current}/${total} (TEST MODE)`);
                }
                
                // Update initial progress
                updateProgress(0, totalPages);
                
                // Generate pages sequentially with retry logic
                for (let i = 0; i < totalPages; i++) {
                    let retryCount = 0;
                    let success = false;
                    ...
                    while (retryCount < maxRetries && !success) {
                        try {
                            ...
                            const pageResult = await generateSingleLovePage({
                                image1: base64Image1,
                                image2: base64Image2,
                                prompt: pagePrompts[i],
                                pageNumber: i + 1,
                                totalPages: totalPages
                            });
                            ...
                            retryCount++;
                            console.log(`✗ Page ${i + 1} failed (Attempt ${retryCount}/${maxRetries}):`, pageError.message);
                            ...
                            if (retryCount < maxRetries) {
                                ...
                                // Modify prompt slightly for retry
                                pagePrompts[i] = pagePrompts[i] + ' Make sure to include both characters clearly in the scene.';
                            } else {
                                console.log(`✗ Page ${i + 1} failed after ${maxRetries} attempts`);
                            }
                        }
                    ...
                    // Small delay between successful pages to avoid rate limiting
                    if (success && i < totalPages - 1) {
                        await delay(1500); // 1.5 second delay between requests
                    }
                }
                
                console.log(`Successfully generated ${generatedPages.length} out of ${totalPages} pages`);
                ...
                window.generatedStoryPages = generatedPages;
                window.storySelections = validation.selections;
                ...
                if (generatedPages.length === totalPages) {
                    alert(`✅ Successfully generated all ${generatedPages.length} pages! (TEST MODE: 3 pages)`);
                } else {
                    alert(`✅ Generated ${generatedPages.length} out of ${totalPages} pages. (TEST MODE)`);
                }
                ...
                if (window.generatedStoryPages && window.generatedStoryPages.length > 0) {
                    alert(`Partially completed: ${window.generatedStoryPages.length} pages generated. Some pages failed: ${error.message} (TEST MODE)`);
                    $('#story-loading').hide();
                    $('#story-results').show();
                    generateLoveBookPreview();
                    $('.add-to-cart-btn').prop('disabled', false).css('opacity', 1);
        ...
        // Generate prompts for 3 pages based on selections
        function generateLovePagePrompts(selections) {
            const prompts = [];
            
            // Page 1: How We Met
            prompts.push(
                `Create a romantic scene showing ${selections.how_we_met.replace('_', ' ')} where two people first meet.`
            );
            
            // Page 2: Favorite Memory
            if (selections.favorite_memories.length > 0) {
                prompts.push(
                    `Create a beautiful memory scene of ${selections.favorite_memories[0].replace('_', ' ')} together.`
                );
            } else {
                prompts.push(
                    `Create a beautiful romantic memory scene.`
                );
            }
            
            // Page 3: Future Dreams
            prompts.push(
                `Create a dream scene of ${selections.future_dreams.replace('_', ' ')} together.`
            );
            
            // Add book style to all prompts
            const styleInstruction = getLoveStyleInstruction(selections.book_style);
            return prompts.map(prompt => `${prompt} ${styleInstruction} (3-page test version)`);
        }

        // Get style instruction based on book style
        function getLoveStyleInstruction(bookStyle) {
            const styles = {
                'classic_romance': 'Style: Classic romantic painting with soft lighting and dreamy atmosphere.',
                'modern': 'Style: Modern digital illustration with clean lines and contemporary aesthetics.',
                'adventure': 'Style: Adventure storybook style with dynamic compositions and vibrant colors.',
                'dreamy': 'Style: Dreamy, magical illustration with ethereal lighting and fantasy elements.'
            };
            
            return styles[bookStyle] || 'Style: Beautiful romantic illustration.';
        }
...
// NEW FUNCTION: Generate preview prompt
function generatePreviewPrompt(selections) {
    // Create a simple preview prompt
    if (selections.how_we_met) {
        return `Create a romantic scene showing ${selections.how_we_met.replace('_', ' ')} where two people first meet. Style: Beautiful romantic illustration. This is a preview page.`;
    } else {
        return `Create a beautiful romantic scene with two people. Style: Romantic illustration. This is a preview page.`;
    }
}

// Also add this for kids story preview
function generateKidsPreviewPrompt(selections) {
    const templateNames = {
        'little_helper': 'The Little Helper',
        'space_explorer': 'The Space Explorer',
        'animal_rescuer': 'The Animal Rescuer',
    ...
    const templateName = templateNames[selections.template] || selections.template;
    return `Create a children's story illustration for ${templateName}. Style: Bright cartoon illustration for kids. This is a preview page.`;
}
       // Add to Cart (from Step 4) - UPDATED for product selection
$wizard.on('click', '.add-to-cart-btn', function(e) {
    e.preventDefault();
    ...
    if (!purchaseProductId || !purchaseOption) {
        alert('Please select a purchase option first. Choose Digital Only (49DH) or Digital + Physical (289DH).');
        return;
    }
    ...
    // Validate story selections based on type
    if (storyType === 'love') {
        const loveValidation = validateLoveStorySelections();
        if (!loveValidation.isValid) {
            alert('Please complete the love story selections:\n\n' + loveValidation.errors.join('\n'));
            return;
        }
    ...
    if (storyType === 'kids') {
        const kidsValidation = validateKidsStorySelections();
        if (!kidsValidation.isValid) {
            alert('Please complete the kids story selections:\n\n' + kidsValidation.errors.join('\n'));
            return;
        }
    ...
    let avatarUrl2 = '';
    let generatedScene = '';
    let generatedSceneLove = '';
    let previewImage = window.previewImage || '';
    
    if (storyType === 'kids') {
        avatarUrl = $('#avatar-kids-url').val();
        generatedScene = generatedSceneUrl || $('#generated-scene-url').val();
        console.log('Kids data - Avatar:', avatarUrl, 'Preview:', previewImage ? 'Yes' : 'No');
    } else if (storyType === 'love') {
        avatarUrl = $('#avatar-love1-url').val();
        avatarUrl2 = $('#avatar-love2-url').val();
        generatedSceneLove = generatedSceneLoveUrl || $('#generated-scene-url-love').val();
        console.log('Love data - Avatar1:', avatarUrl, 'Avatar2:', avatarUrl2, 'Preview:', previewImage ? 'Yes' : 'No');
    }

    // Use SELECTED product ID, not the default one
    const product_id = purchaseProductId; // This is from the selected option
    const quantity = 1;

    console.log('Selected Product ID:', product_id, 'Quantity:', quantity, 'Option:', purchaseOption);
    ...
    $.ajax({
        ...
        data: {
            ...
            avatar: avatarUrl,
            avatar2: storyType === 'love' ? avatarUrl2 : '',
            character_details: characterData,
            generated_scene: generatedScene,
            generated_scene_love: generatedSceneLove,
    ...
    // This needs to be OUTSIDE initWizard() for event delegation to work
    
    // 1. Handle purchase option selection (enables the add to cart button)
    $(document).on('click', '.purchase-option', function() {
        const selectedProductId = $(this).data('product-id');
        const purchaseOption = $(this).data('option');
        const price = $(this).data('price');
    ...
    // 2. FIXED Add to Cart handler with event delegation
    ...
    // 3. Also update preview generation functions to reset button state
    // Add this to both preview generation functions if not already there:
    // In generateSinglePreviewPage() - add this after showing preview:
    // $('#step4-content-love .add-to-cart-btn').prop('disabled', true).css('opacity', 0.7).html('Add to Cart');
    // 
    // In generateKidsPreviewPage() - add this after showing preview:
    // $('#step4-content-kids .add-to-cart-btn').prop('disabled', true).css('opacity', 0.7).html('Add to Cart');
    
        // Run when document is ready
    $(document).ready(function() {
        console.log('Document ready - checking for Book Wizard');
        
        // Only initialize if wizard exists
        if ($('#book-wizard').length) {
            console.log('Book Wizard found - initializing...');
            initWizard();
        } else {
            console.log('Book Wizard not found - skipping initialization');
        }
    });
})(jQuery);